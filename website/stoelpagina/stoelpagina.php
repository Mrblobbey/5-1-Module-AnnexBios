...
$movie_screening_id = 123; // id van de filmvertoning
...
try {
    $stmt = $conn->prepare("
      ...
      WHERE od.movie_screening_id = :msid AND tc.status = 'valid';
    ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            // Order maken
            $o = $conn->prepare("
                INSERT INTO orders (movie_screening_id, customer_id, total_amount, status)
            ...
            foreach ($gekozenStoelen as $uiId) {
                ...
                $checkIfSeatIsAvailable = $conn->prepare("
                    SELECT COUNT(*) AS cnt FROM tickets AS tc
                    JOIN orders AS od ON tc.order_id = od.order_id
                    WHERE od.movie_screening_id = :msid
                ...
                $t = $conn->prepare("
                    INSERT INTO tickets (order_id, row_number, seat_number, price, status)
